...
一个针对 **Clash** (及兼容核心) 的自动化节点工具。它会自动遍历你的代理节点，通过 [IPPure](https://ippure.com/)或者[Ping0](https://ping0.cc/) 检测 IP 纯净度和相关属性，并通过 [ipinfo.io](https://ipinfo.io/) 检测归属地，重命名节点并添加实用的指标（IP 纯净度、Bot 比例(或共享人数)、IP属性/IP来源、归属地国家）`【🟢🟡 住宅|原生|美国🇺🇸】`。
效果展示：
...
### v2.1.0 (2026-04-30)
- **🌍 归属地检测**: 新增基于 [ipinfo.io](https://ipinfo.io/) 的节点归属地检测，节点命名中追加国家中文名和国旗 emoji（如 `|美国🇺🇸`）。
- **Web UI 列**: 结果表格新增"归属地"列，直观展示国旗与国家中文名。
- **可选 Token**: 支持配置 `ipinfo_token`；留空即使用免费额度（50k 次/月）。
- **⏭️ 增量检测**: 新增 `skip_checked` 选项（默认开启），自动识别并跳过已带成功标签的节点，只检测失败节点 / 无标签节点，大幅加速二次检测。
...
- **深度 IP 分析**: 检测 IP 纯净度分数、Bot 比例、IP 属性 (原生/机房)。
- **🌍 归属地检测**: 通过 ipinfo.io 自动识别节点归属国家，在节点名后追加中文国家名与国旗 emoji（如 `|美国🇺🇸`）。
...
4.  **命令行模式 (旧版)**
    ...
    - 编辑 `config.yaml` 填入配置（Web 界面中也可直接设置）：
        ...
        - `ipinfo_token`: 可选，[ipinfo.io](https://ipinfo.io/) API Token，用于归属地检测；留空使用免费额度（50k 次/月）。
...
格式： `【🟢🟡 住宅|原生|美国🇺🇸】` (默认浏览器模式) 或 `【⚪ 机房|广播|香港🇭🇰】` (极速模式)

*   **第 1 个 Emoji (⚪)**: **IP 纯净度** (值越低越好，越低越像真实用户)
*   **第 2 个 Emoji (🟡)**: **Bot 比例** (浏览器模式独有，值越高来自机器人的流量更大更容易弹验证) / 或 **共享人数** (Ping0 数据源独有)
*   **属性**: 住宅 / 机房
*   **来源**: 原生 / 广播
*   **归属地**: 国家中文名 + 国旗 emoji（通过 ipinfo.io 查询）
